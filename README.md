...
ホットペッパーグルメの掲載店台帳を1日24回自動更新し、「ネット予約カレンダーが
使えなくなった店」「解約（掲載自体が終了した店）」の2種類のアタック対象と、
「新しく掲載され、ネット予約も使えるようになった店」（新規開拓リード）を
...
| ファイル | 役割 |
|---|---|
| `index.html` | アタックリストの画面。「予約不可」「解約（掲載終了）」「新規掲載（予約可）」の3タブ（期間・エリア・ジャンル絞り込み、CSV保存） |
| `scripts/hotpepper-roster.mjs` | ホットペッパーAPIで対象県の全掲載店を取得し、店舗ページの表記からネット予約可否をチェック＋掲載終了（解約）＋新規掲載を検出して `data/hotpepper-roster*.json` を更新 |
...
| `scripts/list-newly-listed.mjs` | 台帳から新規掲載（ネット予約可）店をCLI表示・CSV出力するヘルパー |
| `scripts/test-data.mjs` | `data/*.json` の整合性チェック（台帳更新の前後でActionsから実行） |
| `.github/workflows/update-attack-list.yml` | 1日24回自動実行（Actionsのcron。実際の実行時刻はActions側の混雑状況でずれることがある） |
...
| `data/hotpepper-newly-listed*.json` | 台帳から抽出した新規掲載（ネット予約可）店のみの軽量版（`index.html` が読む） |
...
- **解約（掲載終了）検出**：予約可否を問わず、ページ自体が無くなった店を全件検出する。
  ...
  これを解約と誤判定しないため。なお`delistedAt`は「掲載が消えたことにこちらが気づいた日」
  であり、実際の閉店日とは限らない（ホットペッパー側の掲載整理が閉店より遅れることがある）
- **新規掲載（ネット予約可）検出**：解約の逆で、新しく掲載され`reservable: true`が
  確認できた店を新規開拓リードとして検出する。全件チェックは実行のたびに走るため、
  解約検出と違って実際の掲載開始とのズレは運用間隔程度に収まる。台帳がまだ無い最初の
  実行（初期データ移行）で一括登録された店は対象外にしている
