...
**アタックリストに載っている店（`reservationLostAt`あり）は毎回のローテーションで
優先的に再チェックする**（800件枠の先頭を必ず割り当てる）。優先しないと、ネット予約が
再び使えるようになった店が次に選ばれるまで（東京なら最大2週間ほど）アタックリストに
載ったままになってしまうため。再チェックで`reservable: true`が確認できた店は
`reservationLostAt`を削除し、アタックリストから自動的に外れる。
