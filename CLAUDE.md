...
1日6回自動更新し、「ネット予約カレンダーが使えなくなった店」「解約（掲載自体が
終了した店）」の2種類のアタック対象と、「新しく掲載され、ネット予約も使えるように
なった店」（新規開拓リード）をGitHub Pagesで一覧表示する。
...
| `scripts/hotpepper-roster.mjs` | ホットペッパー全掲載店の台帳更新＋ネット予約可否チェック＋解約（掲載終了）検出＋新規掲載検出（`--pref=`で県指定）。Actionsから1日6回、4県分実行 |
...
800件ローテーションで確認（1日6回×800件＝4,800件／日／県。千葉・埼玉は約1日、
神奈川は約2日、店舗数の多い東京は約1週間で一巡）。
...
不要（掲載終了チェックは1日6回・毎回全件走るため）。予約可だった店が解約した場合は、
